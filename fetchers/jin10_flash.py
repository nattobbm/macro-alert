...
_SPEAKER_PAT = re.compile(
    ...
    # 2026-09-09 修：动词表原来把「发表讲话」整体写死，遇到「发表演讲」就配不上，
    # 名字被贪到「拉加德发表」，同一个人当成两个人 → 日历上拉加德 01:00 出现两次。
    # 改成「发表」可选 + 动词单列，讲话/演讲/演说/发言四种写法都归到同一个人。
    r"(?P<name>[一-龥·]{2,6}?)"
    r"(?:(?:发表)?(?:讲话|演讲|演说|发言)|出席|参加|召开|主持|接受采访)"
)
...
def daily_events(j: Jin10, days_back: int = 2) -> list[dict]:
    ...
    dedup = []
    for e in out:
        # 同一个人同一分钟只可能是同一件事，金十不同版本会写成「发表讲话」和
        # 「发表演讲」两条。认人不认标题；认不出人的才退回标题比对。
        k = ((e["date"], e.get("time_bj"), "sp:" + e["speaker"]) if e.get("speaker")
             else (e["date"], e.get("time_bj"), e["title"]))
        if k in seen:
            continue
        seen.add(k)
        dedup.append(e)
